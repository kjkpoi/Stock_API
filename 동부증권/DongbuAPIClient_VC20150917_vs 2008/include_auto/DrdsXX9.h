...
typedef struct {
	...
	DEFINE_DRDS_LINE0("매체구분",			    comgb,        char,   2) //20091110 방현진 <<
	// 20110719 주우주 >>
	DEFINE_DRDS_LINE0("등락부호",                       sign,		  char,  1)
	DEFINE_DRDS_LINE0("전일대비",                       change,       char,  7)
	DEFINE_DRDS_LINE0("등락율",							chgrate,      char,  7)
	// 20120723 ssj - 서버자동주문 >>
	//DEFINE_DRDS_LINE0("filler",						filler,       char,   36) 

	//20121130 이문수 >>
	//DEFINE_DRDS_LINE0("filler",							filler,       char,   35)
...
typedef struct {
	DEFINE_DRDS_LINE0("userid",                         userid,       char,  20)
	DEFINE_DRDS_LINE0("지점번호",                       jijum,        char,   3)
	DEFINE_DRDS_LINE0("계좌번호",                       geja,         char,   8) //8->17
	DEFINE_DRDS_LINE0("종목코드",                       jgcode,       char,   6) //6->12
	DEFINE_DRDS_LINE0("종목명",                         jgname,       char,  14) //14->15
	DEFINE_DRDS_LINE0("금일매수금액",                   buyamt,       char,  12) //12->15
	DEFINE_DRDS_LINE0("금일매도금액",                   sellamt,      char,  12) //12->15
	DEFINE_DRDS_LINE0("금일매도정산금액",               arngamt,      char,  12) //12->15
	DEFINE_DRDS_LINE0("장부수량",                       bookqty,      char,   9) //9->15
	DEFINE_DRDS_LINE0("장부단가",                       bookprc,      char,   7) //7->15
	DEFINE_DRDS_LINE0("평가금액",                       evamt,        char,  12) //12->15
	DEFINE_DRDS_LINE0("평가손익",                       evpl,         char,  12) //12->15
	DEFINE_DRDS_LINE0("당일변동수량",                   tdchngqty,    char,   9) //9->15
	DEFINE_DRDS_LINE0("현재가",                         close,        char,   7)
	DEFINE_DRDS_LINE0("매수호가",                       bid,          char,   7)
	DEFINE_DRDS_LINE0("매도호가",                       offer,        char,   7)
	DEFINE_DRDS_LINE0("상한가",                         hilimit,      char,   7)
	DEFINE_DRDS_LINE0("하한가",                         lolimit,      char,   7)
	DEFINE_DRDS_LINE0("시장소속구분",                   mrktblngtp,   char,   1)
	DEFINE_DRDS_LINE0("당일실현손익",                   tdpl,         char,  12)
	DEFINE_DRDS_LINE0("제비용",                         tax,          char,   9)
	DEFINE_DRDS_LINE0("수익율",                         suikrate,     char,   7)
	DEFINE_DRDS_LINE0("대출일",                         daechul,      char,   8)
	DEFINE_DRDS_LINE0("신용금액",                       sinamt,       char,  12)
	DEFINE_DRDS_LINE0("증거금율",                       bocash,       char,   4)
	DEFINE_DRDS_LINE0("만기일",                         mangiil,      char,   8)
	DEFINE_DRDS_LINE0("잔고구분",                       jangogb,      char,   2) //2->3(안바뀐거 같은데...? 방현진)
	DEFINE_DRDS_LINE0("매도가능수량",                   possqty,      char,   15) //20091110 방현진 <<
} DRDS_XX9_DATA_1;
// 20071130 권휘영 << 
